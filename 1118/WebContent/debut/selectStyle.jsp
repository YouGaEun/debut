...
<!-- page_center영역 -->
  ...
  <%
  				String watchStyle = request.getParameter("watchStyle");
     			String sorting = request.getParameter("sorting");
	  			...
	  			try{//2. try catch문 만듦
	  				...
	  				watches = dao.sortingList3(conn, sorting, watchStyle);
	  			}catch(SQLException e){}
	 
	  %>
	  <c:set var = "watchStyle" value = "<%=watchStyle %>"/>
	  <div class = "title"><h2 style = "color: #810B0C"> <%=watchStyle %> 스타일의 시계</h2></div>
    <div id="orderbyList"><form method = "post" action = "selectStyle.jsp?watchStyle=<%=watchStyle%>"><!-- 재귀호출 -->
...
<fmt:parseNumber var = "row" integerOnly="true" value = "${fn:length(list)/3 }"/>
<c:set var= "height" value = "${row*350+140+200 }"></c:set>
<!--page_footer영역-->

...
public class WatchDao {
	...
	public List<Watch> sortingList3(Connection conn,String sorting,String watchStyle) //스타일별 정렬
			throws SQLException {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		List<Watch> watchList = null;
		try {
			if(sorting == null){
				pstmt = conn.prepareStatement
					("select * from watch where watchStyle=?");
				pstmt.setString(1, watchStyle);
			}else if(sorting.equals("lowPrice")){
				pstmt = conn.prepareStatement("select * from watch where watchStyle=? order by price");
				pstmt.setString(1, watchStyle);
			}else if(sorting.equals("highPrice")){
				pstmt = conn.prepareStatement("select * from watch where watchStyle=? order by price desc");
				pstmt.setString(1, watchStyle);
			}else if(sorting.equals("new")){
				pstmt = conn.prepareStatement("select * from watch where watchStyle = ? order by watchTime desc");
				pstmt.setString(1, watchStyle);
			}else if(sorting.equals("fav")){
				pstmt = conn.prepareStatement("select * from watch where watchStyle = ? order by watchClick desc");
				pstmt.setString(1, watchStyle);
			}
			rs  = pstmt.executeQuery(); 
			watchList = new ArrayList<Watch>();
